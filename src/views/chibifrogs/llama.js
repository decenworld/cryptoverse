...
const contractAddress = '0xCdb4c301F33A3BA5b3E5E7eba1a94f0cAe321D6A';
...
const [mintAmount, setMintAmount] = useState(3);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'ChibifrogsNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'ChibifrogsNFT', 'LlamaStakingNFT');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('ChibifrogsNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('ChibifrogsNFT', 'LlamaStakingNFT'));
    }
  }
  ...
  const approve = async () => {
    await tombFinance.approve('ChibifrogsNFT', 'LlamaStakingNFT');
  }

  const mint = async (amount) => {
    console.log(account);
    await tombFinance.mintchibifrogsNFT(account, amount);
    ...
    <Switch>
      <Page>
        <Route exact path={path}>
          ...
          <h2 style={{ textAlign:'center', marginBottom: '5px'  }}>Chibi Frogs</h2>
          ...
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span style={{ fontSize: '36px' }}>
              { parseInt(nftStakedTotalSupply * 100 / nftTotalSupply) } % Frogs STAKED
            </span>
