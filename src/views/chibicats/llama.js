...
const contractAddress = '0xc65c2dab87f870592880978bee5648118232900e';
...
const [mintAmount, setMintAmount] = useState(3);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'ChibicatsNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'ChibicatsNFT', 'LlamaStakingNFT');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('ChibicatsNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('ChibicatsNFT', 'LlamaStakingNFT'));
    }
  }
  ...
  const approve = async () => {
    await tombFinance.approve('ChibicatsNFT', 'LlamaStakingNFT');
  }

  const mint = async (amount) => {
    console.log(account);
    await tombFinance.mintchibicatsNFT(account, amount);
